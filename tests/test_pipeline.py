...
class Pipe(unittest.TestCase):
    ...
    def test_surge_detects_spike_and_emerging(self):
        ...
        rows.append({"date":"2026-08-17","theme":"immigration & migrants","volume":12,"share":0.08,"outlet_breadth":4})
        th.write_text("".join(json.dumps(r)+"\n" for r in rows))
        run("surge.py","--data-dir",self.tmp,"--min-baseline","3","--min-confident","6","--min-articles","10")
